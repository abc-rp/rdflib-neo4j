...
def neo4j_container():
    if not LOCAL:
        container = (
            Neo4jContainer(image="neo4j:5.7.0-enterprise")
...
def neo4j_driver(neo4j_container):
    ...
    if not LOCAL:
        ...
        driver = GraphDatabase.driver(
            auth_data["uri"], auth=(auth_data["user"], auth_data["pwd"])
        )

    # initialize n10s procs
    driver.execute_query(
        "CREATE DATABASE " + RDFLIB_DB + " IF NOT EXISTS WAIT", database_="system"
    )
