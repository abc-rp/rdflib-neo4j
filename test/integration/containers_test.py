...
def test_import_person(neo4j_driver, graph_store):
    ...
    records = neo4j_driver.execute_query(
        "CALL n10s.rdf.import.inline($payload, 'Turtle')", payload=rdf_payload
    )
    ...
    records_from_rdf_lib, summary, keys = neo4j_driver.execute_query(
        GET_DATA_QUERY, database_=RDFLIB_DB
    )
...
def test_read_file_batched(neo4j_driver, graph_store_batched):
    ...
    records_from_rdf_lib, records, _, _ = read_file_n10s_and_rdflib(
        neo4j_driver, graph_store_batched, True
    )
    assert len(records_from_rdf_lib) == len(records)
    for i in range(len(records)):
        assert records_equal(records[i], records_from_rdf_lib[i])
